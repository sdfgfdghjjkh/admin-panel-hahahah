...
function Data() {
  ...
  const fetchAllCollectionsCategories = async () => {
    ...
    try {
      ...
      const q2 = query(
        collection(db, "sales"),
        where("brand.category", "==", categoryValue)
      );
  ...
  const delAllCollectionsCategories = async () => {
    ...
    try {
      const q1 = query(
        collection(db, "brands"),
        where("category", "==", categoryValueDel)
      );
      const querySnapshot1 = await getDocs(q1);
      querySnapshot1.forEach((doc) => {
        setBrandId(doc.id);
      ...
      const q2 = query(
        collection(db, "sales"),
        where("brand.category", "==", categoryValueDel)
      );
      const querySnapshot2 = await getDocs(q2);
      querySnapshot2.forEach((doc) => {
        setSaleId(doc.id);
  ...
  return {
    columns: [
      ...
      { Header: "Mamont name", accessor: "category", align: "left" },
            ...
            <>
              ...
              >
                ...
                {/* <MDButton
                  ...
                  onClick={() => categoryModalOpen(items)}
                >
